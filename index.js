window.onload = function () {
  ...
  nullification(); // заполненили массивы
  ...
  function movement(el, robot = false) {
    counter++;
    let coordinateEl; // здесь будет хранится элемент передвигаемый
    let coordinateZe; // здесь будет хранится пустая ячейка
    let zero = document.getElementById(0); // получим пустую ячейку

    document.querySelector(".score").textContent = counter; // выведем счет
    ...
    if (!checker(coordinateEl, coordinateZe)) {
      ...
      arrGameNumsIn[coordinateEl[0]][coordinateEl[1]] = 16; // 16 это нулевой блок
      arrGameNumsIn[coordinateZe[0]][coordinateZe[1]] = Number(el.textContent); // меняем местами контент
      zero.outerHTML = `<td id = ${el.textContent}>${el.textContent}</td>`;
      el.outerHTML = `<td id = 0 style="background-color:hsla(200, 60%, 30%, 0.5);"></td>`; //hsla(200, 60%, 30%, 0.3)
      if (!robot) {
        if (winCheck())
        return;
      } //проверяем на победку
    }
  ...
  function checker(coordinateEl, coordinateZe) {
    let possibleElems = [[], [], [], []]; // создаем массив элементо вокруг пустой ячейки
  ...
  function winCheck() {
    ...
    Youre score is ${counter}`);
      document.querySelector('.play-table').innerHTML = `<div class="win">You've Won!? Congrats,
      Youre score is ${counter}!</div><div class="image"></div>`
      return true;
    }
  }

  /**
   * будущая фича на подсветку возможных полей
   */
